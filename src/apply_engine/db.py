...
def start_run(db: Client, kind: str) -> int:
    response = db.table("runs").insert({"kind": kind}).execute()
    return response.data[0]["id"]


def finish_run(
    db: Client, run_id: int, *, ok: bool,
    stats: dict[str, Any] | None = None, error: str | None = None,
) -> None:
    """Always called, success or failure.

    The watchdog task checks for today's ingest row. A run that dies without
    writing one looks identical to a run that never fired, and a pipeline that
    fails silently is worse than no pipeline.
    """
